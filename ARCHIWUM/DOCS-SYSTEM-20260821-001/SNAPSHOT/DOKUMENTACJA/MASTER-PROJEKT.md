...
| P1 | dokładna klasa operacji | 10 s | `TO RECOVER` |
| P2 | dokładna klasa operacji | 45 s | `TO RECOVER` |
| P3 | dokładna klasa operacji | 180 s | `TO RECOVER` |
...
| `companies.db` | dane/fundament klasyfikacji spółek | `CONFIRMED` jako baza obszaru spółek; pełny schemat `TO RECOVER` |
| druga baza biznesowa | audyt monitorował łącznie 2 bazy biznesowe | nazwa i pełna rola `TO RECOVER` |
...
### R1.5 — Przywrócenie działania wykresów — `NEXT`

Cel: kontrolowanie uruchomić zaakceptowane r599 bez zmiany managera ON i zweryfikować starszą ścieżkę `/candles/{symbol}` dla jednego symbolu. Integracja finalnego V3 pozostaje osobną paczką.
...
6. `NEXT` — pionowy przepływ: jedna spółka → dane 5 TF → payload → wykres w głównej aplikacji;
...
Aktualny workspace zawiera jedynie pliki kontekstu projektu, a nie lokalne repo `C:\Skaner wykresów`. Faktyczne utworzenie powyższej struktury w repo produkcyjnym jest `PENDING` i wymaga dostępu do tego repo.
...
|---|---|---|---|
| r595 | `TO RECOVER` | brak wystarczających danych w bieżącym materiale | `ARCHIWUM/r595/` — do utworzenia/odzyskania |
| r596 | `TO RECOVER` | brak wystarczających danych | `ARCHIWUM/r596/` |
| r597 | `TO RECOVER` | brak wystarczających danych | `ARCHIWUM/r597/` |
| r598 | `PARTIAL` | istnieje warstwa telemetryczna/pomiarowa; flaga pozostaje 0; pełny AS-BUILT do odzyskania | `ARCHIWUM/r598/` |
...
- odzyskać znaczenie P1, P2 i P3 dla operacji runtime.
...
CURRENT STAGE:
AFTER r599 SUCCESSFUL CONTROLLED ACTIVATION;
MANAGER ON; RUNTIME CURRENTLY OFF;
PROJECT RECOVERY AND DOCUMENTATION SYSTEM INSTALLED

NEXT AUTHORIZED PROGRAM STAGE:
CHARTS-RESTORE PACKAGE:
CONTROLLED START WITHOUT CONFIG CHANGE,
ONE-SYMBOL /candles AND UI VERIFICATION
...
Po instalacji bazową lokalizacją całej dokumentacji jest:
